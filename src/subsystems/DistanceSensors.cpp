...
// Returns the distance reading of a specific sensor
float DistanceSensors::getSensorDistance(DistanceSensors::SENSOR sensor) {
    VL53L4CD selectedSensor = distanceSensors[(int)sensor];

    if (selectedSensor.ranging_data.range_status == 0) {
        return selectedSensor.ranging_data.range_mm;
    } else {
        // Indicates an error
        return -1;
    }  
...
void DistanceSensors::interpolate() {
